...
2. 每个编号必须恰好出现一次，不许漏、不许重复、不许编造。涉及几个世界的块（比如总设定），归到最主要的那个。
3. 类型是「设定笔记」「提纲」的块也要归到世界里。
4. 新世界写 name、reason 和 scenes，不带 id。同一个世界的块写在一起：新世界之间不能重名，也不能跟已有的世界重名。
5. 下面列了「已有的世界」时，属于它的块写它的 id（照抄编号，写成字符串），比如 {"id": "W-01", "scenes": ["S-0003"]}。已有的世界是「（无）」时只写新世界。
6. reason 用一两句话说明这个世界的判定依据，引用原文线索。
7. $unit_rule
