...
3. facts 里每条的 quote 必须是从片段中逐字复制的一句话（或一句中连续的一段），用来证明这条事实。不要改字，不要把繁体改成简体。quote 去掉标点后至少要有 4 个字，太短（比如「他」「是」）没法核对。facts 的 subject 要照片段原文的写法填一个具体的人名/地名/组织名，最好就是 characters / locations / organizations 里列出的某一个，不要写成「林清的父亲」这种描述性的说法。
